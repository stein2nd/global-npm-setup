# Global npm Package Installer - Modification
...
- **タイトル** — 各ファイルの1行目に `# Global npm Package Installer - …` 形式で記載する。
...
| [mod-os-agnostic-windows.md](./mod-os-agnostic-windows.md) | Windows 11 向けセットアップ・制約 | 確定 |
...
v2 初期リリース（方式 A: パッケージ同梱）完了後に着手する。
...
| 3 | `package.json` v2 化 | `@s2j/global-npm`、GPL-3.0-or-later、`bin` 定義 |
| 4 | `LICENSE` 追加 | GPL v3 全文 |
| 5 | README 更新 | OS 別手順、v1 からの移行 |
| 6 | `install-global.zsh` 削除 | レガシー廃止 |
| 7 | 初回 npm publish | `@s2j/global-npm` v2.0.0 |
| 8 | Windows 11 実機確認 | check / update / install、CLI on PATH |
