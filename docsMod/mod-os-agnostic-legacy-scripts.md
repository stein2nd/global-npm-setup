# Global npm Package Installer - レガシースクリプト廃止（脱 OS 依存改修）

## 背景

v1 では `./install-global.zsh` と `~/bin/global-npm`（Zsh ラッパー）が install / 更新フローの入口だった。
v2 では `@s2j/global-npm` CLI に統一し、両者を **廃止** する。
...
| 利点 | 説明 |
|------|------|
| 極めて単純 | 12 行。依存は zsh + npm のみ |
| リポジトリ同梱 | Git で追跡。npm publish 不要 |
| 自己位置解決 | `cd "$(dirname "$0")"` で setup dir を確定 |
| PATH 不要 | `./install-global.zsh` で直接実行可能 |
| install 専用 | 更新確認・書き換えと責務が分離 |
| オフライン寄り | 既に clone 済みならネットワーク最小限 |

### 欠点

| 欠点 | 説明 |
|------|------|
| B 型 install | 依存 CLI の `bin` が PATH に載らない（[mod-os-agnostic-install.md](./mod-os-agnostic-install.md) 参照） |
| Zsh 専用 | Windows / bash では動かない |
| 二重入口 | 本番は `ncu:install` 経由のことが多く、役割が曖昧 |
...
| SETUP_DIR の曖昧さ | README では `~/bin/global-npm` に置く想定だが、`SETUP_DIR="${0:A:h}"` だと `~/bin` を指す。正しく動かすには setup 内に置いて symlink する等の追加手順が必要 |
...
| Mac 限定の再現 | 勤務先 Windows 11 にはそのまま持ち込めない |
...
| 利点 | 説明 |
|------|------|
| OS 非依存 | Node + npm のみ。macOS / Windows 11 で同一 |
| 入口の一本化 | check / update / install が 1 実装（`bin/global-npm.js`） |
...
| 自己更新可能 | `@s2j/global-npm` を `dependencies` に含め、C 型 install で自身も更新 |
...
| npm publish 依存 | dependencies 一覧の変更は publish（または `npm link` 開発）が必要 | ◎ — [mod-npm-publish.md](./mod-npm-publish.md) 参照 |
| ネットワーク | 初回・更新時に registry 到達が前提 | ◎ |
| 純 dotfiles からの距離 | 「clone するだけ」より npm パッケージとしての運用に寄る | ◎ |
| カスタム一覧 | 勤務先だけ別 pkg 集合にするには fork か方式 B が必要 | △ — v2 残タスクで方式 B を実装予定 |
...
| 入口数 | install のみ（別系統） | 3 操作 | 3 操作（統一） |
...
| 保守箇所 | 1 zsh | zsh + scripts + jq | 1 Node CLI |
| npm 公開 | 不要 | 不要 | 必要 |
...
- [mod-os-agnostic-cli.md](./mod-os-agnostic-cli.md) — v2 CLI 仕様
- [mod-os-agnostic-install.md](./mod-os-agnostic-install.md) — C 型 install
- [mod-npm-publish.md](./mod-npm-publish.md) — publish 依存の受け入れ理由

## ステータス

**確定** — 実装完了後 `docs/legacy-scripts.md` へ移動する。
