...
| 物理的なアーカイブ | `$SETUP_DIR/package.json` | ncu / install が読む実効マニフェスト |
...
global-npm install               # 物理的なアーカイブ化 dependencies を global install
```

初回の `global-npm install` で `~/.config/global-npm/` (Windows 11では `%APPDATA%\global-npm\`) に物理的なアーカイブ化 `package.json` が生成されます。
...
* **update:** 物理的なアーカイブ化 `dependencies` / `devDependencies` の range を更新。
* **install:** 物理的なアーカイブ化 **dependencies** のみ global install。
...
| 同梱 `package.json` を直接 ncu | 物理的なアーカイブを ncu |
...
3. `global-npm sync` で 物理的なアーカイブを生成する。
