...
v1ではプロジェクト名 `global-npm-packages`、ローカル配置 `~/dotfiles/setup/`、Zsh ラッパー `~/bin/global-npm` という構成でした。
v2では Windows 11 (勤務先) でも同じ操作感で使えるよう npm パッケージとして CLI を配布し、`@s2j/docs-linter` と同様の更新管理を行います。
