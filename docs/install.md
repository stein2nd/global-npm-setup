...
v2.1以降、install の入力は **物理的なアーカイブ化 `package.json`**、`$SETUP_DIR/package.json` です ([layout.md](./layout.md))。
...
1. `syncManifest()` で物理的なアーカイブ化 `package.json` を最新化する。
2. 物理的なアーカイブの `dependencies` を Node.js で読み込む。
...
|------|----------------|-------------------|
| `check` / `update` (ncu) | 対象 | 対象。物理的なアーカイブにマージ済みの分 |
| `install` | 対象 | **対象外** |

`user-deps.json` の `devDependencies` は ncu 管理用に物理的なアーカイブにマージするが、global install はしません。

## ncu との整合

| サブコマンド | 操作対象 | ncu / npm の入力 |
|--------------|----------|------------------|
| `global-npm check` | 更新確認のみ | 物理的なアーカイブ化 `package.json` |
| `global-npm update` | バージョン範囲の書き換え | 物理的なアーカイブ化 `package.json` |
| `global-npm install` | グローバルインストール | 物理的なアーカイブ化 **dependencies** のみ |

### 整合のポイント

* **install** が入れるパッケージ集合 = 物理的なアーカイブの `dependencies`。
* **check / update** は物理的なアーカイブの `dependencies` + `devDependencies` を読む。
...
global-npm update   # 物理的なアーカイブの semver 範囲を更新
...
| v2.1 | 物理的なアーカイブ (`$SETUP_DIR/package.json`) |
