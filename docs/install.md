...
* スコープ付き名称も `name@version` 形式で `npm install -g` に渡す (例: `@s2j/docs-linter@1.0.17`)。version は range から `npm view` で解決する。
* `update` で書き換えた semver 範囲が、直後の `install` で registry 上の最新版として反映される。
